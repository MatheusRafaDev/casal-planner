        ...
        public async Task<(ResumoDto Resumo, ComparativoDto Comparativo)> ObterResumoAgregadoAsync(string usuarioId)
        {
            ...
            var pipeline = new[]
            {
                ...
                new BsonDocument("$project", new BsonDocument
                {
                    ...
                    { "IsMesAtual", new BsonDocument("$and", new BsonArray { new BsonDocument("$gte", new BsonArray { new BsonDocument("$ifNull", new BsonArray { "$DataCompra", "$CreatedAt" }), new BsonDateTime(inicioMesAtual) }), new BsonDocument("$lt", new BsonArray { new BsonDocument("$ifNull", new BsonArray { "$DataCompra", "$CreatedAt" }), new BsonDateTime(inicioProximoMes) }) }) },
                    { "IsMesPassado", new BsonDocument("$and", new BsonArray { new BsonDocument("$gte", new BsonArray { new BsonDocument("$ifNull", new BsonArray { "$DataCompra", "$CreatedAt" }), new BsonDateTime(inicioMesPassado) }), new BsonDocument("$lt", new BsonArray { new BsonDocument("$ifNull", new BsonArray { "$DataCompra", "$CreatedAt" }), new BsonDateTime(inicioMesAtual) }) }) },
                    { "IsMesRetrasado", new BsonDocument("$and", new BsonArray { new BsonDocument("$gte", new BsonArray { new BsonDocument("$ifNull", new BsonArray { "$DataCompra", "$CreatedAt" }), new BsonDateTime(inicioMesRetrasado) }), new BsonDocument("$lt", new BsonArray { new BsonDocument("$ifNull", new BsonArray { "$DataCompra", "$CreatedAt" }), new BsonDateTime(inicioMesPassado) }) }) },
                }),
                new BsonDocument("$group", new BsonDocument
                {
                    ...
                    { "MA_TotalGeral", new BsonDocument("$sum", new BsonDocument("$cond", new BsonArray { new BsonDocument("$and", new BsonArray { "$IsMesAtual", "$Comprado" }), "$ValorTotal", 0 })) },
                    { "MA_TotalVR", new BsonDocument("$sum", new BsonDocument("$cond", new BsonArray { new BsonDocument("$and", new BsonArray { "$IsMesAtual", new BsonDocument("$eq", new BsonArray { "$Pagamento", "vr" }), "$Comprado" }), "$ValorTotal", 0 })) },
                    { "MA_TotalNormal", new BsonDocument("$sum", new BsonDocument("$cond", new BsonArray { new BsonDocument("$and", new BsonArray { "$IsMesAtual", new BsonDocument("$ne", new BsonArray { "$Pagamento", "vr" }), "$Comprado" }), "$ValorTotal", 0 })) },
                    ...
                    { "MP_TotalGeral", new BsonDocument("$sum", new BsonDocument("$cond", new BsonArray { new BsonDocument("$and", new BsonArray { "$IsMesPassado", "$Comprado" }), "$ValorTotal", 0 })) },
                    { "MP_TotalVR", new BsonDocument("$sum", new BsonDocument("$cond", new BsonArray { new BsonDocument("$and", new BsonArray { "$IsMesPassado", new BsonDocument("$eq", new BsonArray { "$Pagamento", "vr" }), "$Comprado" }), "$ValorTotal", 0 })) },
                    { "MP_TotalNormal", new BsonDocument("$sum", new BsonDocument("$cond", new BsonArray { new BsonDocument("$and", new BsonArray { "$IsMesPassado", new BsonDocument("$ne", new BsonArray { "$Pagamento", "vr" }), "$Comprado" }), "$ValorTotal", 0 })) },
                    ...
                    { "MR_TotalGeral", new BsonDocument("$sum", new BsonDocument("$cond", new BsonArray { new BsonDocument("$and", new BsonArray { "$IsMesRetrasado", "$Comprado" }), "$ValorTotal", 0 })) },
                })
            };
