...
export const tokenStorage = {
  ...
  set:    async (token: string) => await SecureStore.setItemAsync(TOKEN_KEY, token),
  remove: async ()      => { 
    await SecureStore.deleteItemAsync(TOKEN_KEY); 
    await SecureStore.deleteItemAsync(PESSOA_KEY); 
  },
  exists: async ()      => !!(await SecureStore.getItemAsync(TOKEN_KEY)),
};

export const pessoaStorage = {
  get:    async ()        => (await SecureStore.getItemAsync(PESSOA_KEY)) || null,
  set:    async (pessoa: string | null) => pessoa
    ? await SecureStore.setItemAsync(PESSOA_KEY, pessoa)
    : await SecureStore.deleteItemAsync(PESSOA_KEY),
  remove: async ()        => await SecureStore.deleteItemAsync(PESSOA_KEY),
};
