...
import {
  ...
  ShoppingBag, Store, ExternalLink, GripVertical,
...
const isAddedToday = (createdAt) => {
  ...
  const added = new Date(createdAt); added.setHours(0, 0, 0, 0);
...
const ItemRow = memo(({
  ...
  onToggleComprado, onEditItem, onDeleteItem, onDragStart, onDragEnd,
  theme,
}) => {
    ...
    >
      <S.ItemMainRow>
        ...
        <S.CheckboxButton
          ...
          onClick={(e) => { e.stopPropagation(); onToggleComprado(item.id, item.comprado); }}
          theme={theme}
          disabled={disabled}
        >
        ...
        <S.PriorityBadgeSmall $priority={item.prioridade} $color={prioridadeConfig.color} theme={theme} title={`Prioridade: ${prioridadeConfig.label}`}>
      ...
      <S.ItemDetailsRow>
        ...
        <S.ItemActions>
          {item.linkProduto && (
            <S.ItemActionButton onClick={handleOpenLink} theme={theme} variant="link" title="Ver na loja" disabled={disabled}>
              <ExternalLink size={14} />
            </S.ItemActionButton>
          )}
          <S.ItemActionButton onClick={() => onEditItem(item)} theme={theme} variant="edit" title="Editar item" disabled={disabled}>
            <Pencil size={14} />
          </S.ItemActionButton>
          <S.ItemActionButton variant="delete" onClick={() => onDeleteItem(item)} theme={theme} title="Excluir item" disabled={disabled}>
            <Trash2 size={14} />
          </S.ItemActionButton>
        </S.ItemActions>
      </S.ItemDetailsRow>
...
const CategoriaCard = ({
  categoria, itens, onAddItem, onUpdateItem, onDeleteItem,
  onDeleteCategoria, onEditCategoria, onItemDragStart, onItemDragEnd,
  onItemDrop, draggedItemId, theme, onToggleComprado, isLoading = false,
}) => {
  ...
  const { handleDeleteCategoria, handleEditCategoria } =
    useCategoryActions(categoria, itens, theme, onDeleteCategoria, onEditCategoria);
  ...
  const totalCategoria = useMemo(() => itens.reduce((acc, i) => acc + (i.preco * i.quantidade || 0), 0), [itens]);
  const itensComprados = itens.filter(i => i.comprado).length;
  const progresso = itens.length > 0 ? (itensComprados / itens.length) * 100 : 0;
  const totalGasto = useMemo(() => itens.filter(i => i.comprado).reduce((acc, i) => acc + (i.preco * i.quantidade || 0), 0), [itens]);
  ...
  const handleAddItem = useCallback(async () => {
    if (isLoading || isSaving) return;
    setIsSaving(true);
    try { await onAddItem(categoria.id); } finally { setTimeout(() => setIsSaving(false), 500); }
  ...
  return (
    <S.CardContainer theme={theme} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} $isDragOver={isDragOver}>
      <S.CardHeader color={categoria.bg} theme={theme}>
        <S.HeaderLeft>
          <S.DragHandle theme={theme}><GripVertical size={16} /></S.DragHandle>
          <S.Icon theme={theme}>{categoria.icon}</S.Icon>
          <S.TitleSection>
            <S.Title theme={theme} title={categoria.nome}>{categoria.nome}</S.Title>
            <S.Subtitle>
              ...
              {categoria.metaOrcamento > 0 && (
                <S.MetaBadge
                  $excedeu={excedeuMeta} $proximo={proximoLimite} theme={theme}
                ...
                >
                  <span>🎯</span>{formatarMoeda(categoria.metaOrcamento)}
                </S.MetaBadge>
              )}
            </S.Subtitle>
          </S.TitleSection>
        ...
        <S.HeaderActions>
          ...
          <S.IconButton onClick={handleEditCategoria} theme={theme} title="Editar categoria" disabled={disabled}><Pencil size={16} /></S.IconButton>
          <S.IconButton danger onClick={handleDeleteCategoria} theme={theme} title="Excluir categoria" disabled={disabled}><Trash2 size={16} /></S.IconButton>
                ...
                <S.SortButtonsGroup>
                  {[
                    ...
                    <S.SortButton key={field} $active={sortBy === field} onClick={() => handleSort(field)} disabled={disabled} theme={theme}>
                      ...
                      {sortBy === field && <S.SortIcon>{sortOrder === "asc" ? <ArrowUp size={14} /> : <ArrowDown size={14} />}</S.SortIcon>}
                    </S.SortButton>
                  ))}
                </S.SortButtonsGroup>
              </S.SortBar>
      ...
      <S.CategoryFooter theme={theme}>
        <S.CategoryStats>
          <S.StatItem theme={theme}><span>📦 Comprados:</span><strong>{itensComprados}/{itens.length}</strong></S.StatItem>
          <S.StatItem theme={theme}><span>📊 Progresso:</span><strong>{progresso.toFixed(0)}%</strong></S.StatItem>
          <S.StatItem theme={theme}><span>💰 Gasto:</span><strong>{formatarMoeda(totalGasto)}</strong></S.StatItem>
        </S.CategoryStats>
      </S.CategoryFooter>
    </S.CardContainer>
  );
